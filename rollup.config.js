...
function createRelativeImportPath(sourcePath, targetPath) {
  let importPath = path
    .relative(
      ...
      path.join(exifrDir, "./src/", targetPath)
    )
    .replace(/\\/g, "/");
...
function replaceBuiltinsWithIePolyfills() {
  ...
  return {
    async transform(code, filePath) {
      ...
      let importLine = `import {${polyfillKeys.join(
        ", "
      )}} from '${polyfillPath}'`;
      return importLine + "\n" + code;
    },
...
function replaceFetchPolyfills() {
  return {
    async transform(code, filePath) {
      ...
      let polyfillPath = createRelativeImportPath(
        filePath,
        "polyfill/fetch-xhr.mjs"
      );
